...
async def cmd_help(update: Update, context: CallbackContext) -> None:
    ...
    help_text = (
        ...
        "/addpair - Добавить новую пару для отслеживания\n"
        "/mypairs - Просмотреть и управлять отслеживаемыми парами\n"
        "/price - Получить текущую цену для пары\n\n"
        "📈 *Как использовать бот:*\n"
        "1. Нажмите /addpair для добавления новой пары\n"
        ...
        "⚙️ *Дополнительно:*\n"
        "- Используйте /mypairs для управления парами\n"
        "- Нажмите /price для быстрой проверки текущей цены"
    )
...
async def cmd_my_pairs(update: Update, context: CallbackContext) -> None:
    ...
    if not pairs:
        await update.message.reply_text(
            "У вас нет отслеживаемых пар. Используйте /addpair чтобы добавить.",
            reply_markup=get_main_keyboard()
        )
        return
